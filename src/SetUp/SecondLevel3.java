...
public class SecondLevel3 extends BasicGameState{
    ...
    Image SScreen,Quddus,QuddusL,Bullet,Gem;
    Image Flag,Icicle,UFO;
    Image VBird,VCrow,Lives;
    Image IceVill,IceVill2;
    Image Pause,Instruct,Win;
    float qx=20,qy=360;
    float bx=0,by=1000;
    float e1x=300,e1y=350;
    float e2x=500,e2y=355;
    float e3x=625,e3y=350;
    ...
    public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException {
        ...
        if(die==false){
        if(turn==false){
            IceVill.draw(e1x,e1y);
            e1x-=0.5;
            enemyPoly1.setX(e1x);
        }
        if(turn==true){
            IceVill.getFlippedCopy(true, false).draw(e1x,e1y);
            e1x+=0.5;
            enemyPoly1.setX(e1x);
        }
        if(turn2==false){
        ...
        if(turn3==false){
            IceVill2.draw(e3x,e3y);
            e3x-=0.5;
            enemyPoly3.setX(e3x);
        }
        if(turn3==true){
            IceVill2.getFlippedCopy(true, false).draw(e3x,e3y);
            e3x+=0.5;
            enemyPoly3.setX(e3x);
        }
        }
